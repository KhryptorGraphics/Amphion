...
class PresetCreate(BaseModel):
    ...
    model_type: str = Field(..., description="Model type (e.g., tts, vc, svc)")
...
def list_presets(
    model_id: Optional[str] = None, model_type: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
    List all presets, optionally filtered by model_id and/or model_type.
    ...
    if model_type:
        presets = [p for p in presets if p.get("model_type") == model_type]
...
async def get_presets(
    ...
    """
    List all saved presets, optionally filtered by model_id and/or model_type.

    Args:
        model_id: Optional query parameter to filter presets by model
        model_type: Optional query parameter to filter presets by model type

    Returns:
        List of presets sorted by creation date descending
    """
    presets = list_presets(model_id=model_id, model_type=model_type)
...
async def create_preset(preset_create: PresetCreate) -> Preset:
    ...
    Args:
        preset_create: Preset creation data including name, model_id, model_type, and parameters
    ...
    preset = {
        ...
        "model_type": preset_create.model_type,
