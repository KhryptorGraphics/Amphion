...
const PARAMETER_HELP = {
  noiseScale: "Stochastic duration predictor noise scale (flow noise).",
  noiseScaleW: "Stochastic duration predictor noise scale (duration noise).",
...
export default function VITSPage() {
  ...
  const [params, setParams] = useState<VITSParams>({
    noiseScale: 0.667,
    noiseScaleW: 0.8,
    lengthScale: 1.0,
    voiceId: "default",
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      formData.append("noise_scale_w", params.noiseScaleW.toString());
      ...
      const response = await fetch("/api/tts/vits", { method: "POST", body: formData });
      clearInterval(progressInterval);
      if (!response.ok) throw new Error("Generation failed");
      const blob = await response.blob();
      setGeneratedAudio(URL.createObjectURL(blob));
      setProgress(100);
      toast({ title: "Success", description: "Audio generated with VITS" });
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold">VITS</h1>
        <p className="text-muted-foreground">Variational Inference with adversarial learning for end-to-end Text-to-Speech</p>
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-6">
              {[
                ...
                { key: "noiseScaleW", label: "Noise Scale W", min: 0, max: 1, step: 0.01 },
          ...
          <Button onClick={handleGenerate} disabled={isGenerating || !text.trim()} className="w-full h-12">
            {isGenerating ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Generating...</> : <><Play className="mr-2 h-5 w-5" />Generate with VITS</>}
          </Button>
