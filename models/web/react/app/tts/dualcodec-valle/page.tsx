...
interface DualCodecVALLEParams {
  temperature: number;
  topK: number;
  topP: number;
  maxLength: number;
...
const PARAMETER_HELP = {
  ...
  numSamples: "Number of candidate samples to generate.",
};
...
export default function DualCodecVALLEPage() {
  ...
  const [params, setParams] = useState<DualCodecVALLEParams>({
    temperature: 0.7,
    topK: 50,
    topP: 0.9,
    maxLength: 2048,
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/tts/dualcodec-valle", {
        method: "POST",
        body: formData,
      });
      ...
      toast({
        title: "Success",
        description: "Audio generated successfully with DualCodec-VALLE",
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleDownload = () => {
    ...
    a.download = `dualcodec-valle-${Date.now()}.wav`;
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold tracking-tight">DualCodec-VALLE</h1>
        <p className="text-muted-foreground">
          Neural Codec Language Model with dual-codec representation
        </p>
      </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2">
                    Number of Samples
                    <HelpTooltip content={PARAMETER_HELP.numSamples} />
                  </Label>
                  <span className="text-sm text-muted-foreground w-12 text-right">
                    {params.numSamples}
                  </span>
