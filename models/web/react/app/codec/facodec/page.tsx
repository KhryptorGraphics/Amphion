...
import { Download, ArrowLeft, Loader2, Upload, FileAudio, ArrowRightLeft, Copy, Check } from "lucide-react";
import Link from "next/link";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

export default function FACodecPage() {
  ...
  const handleEncode = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/codec/facodec/encode", {
        method: "POST",
        body: formData,
      });
      ...
      toast({ title: "Success", description: "Audio encoded successfully with FACodec" });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleDecode = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/codec/facodec/decode", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownload = () => {
    ...
    a.href = resultAudio;
    a.download = `facodec-decoded-${Date.now()}.wav`;
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold tracking-tight">FACodec</h1>
        <p className="text-muted-foreground">Factorized neural audio codec - decompose audio into content, timbre, and prosody</p>
      </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Upload Audio
                  <HelpTooltip content="Upload audio file to encode into factorized tokens (content, timbre, prosody)" />
                </CardTitle>
              </CardHeader>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <Textarea
                  value={encodedTokens}
                  readOnly
                  placeholder="Encoded tokens (content, timbre, prosody) will appear here..."
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <FileAudio className="h-5 w-5" />
                  Input Tokens
                  <HelpTooltip content="Enter factorized tokens (JSON format) to decode into audio" />
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <Textarea
                  ...
                  placeholder='{"content": [[1,2,3]], "timbre": [[4,5,6]], "prosody": [[7,8,9]]}'
                  className="min-h-[200px] font-mono text-xs"
                />
